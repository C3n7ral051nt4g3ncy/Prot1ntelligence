...
print("- ProtonMail account existence, Creation date (the account creation date is reliable if we are sure the targeted user didn't generate a new PGP key since account creation)")
print("- User PGP Key, Key creation date, Key Type: RSA 4096 or ECC Curve25519, you can also download the PGP Key download and add to your KeyChain to send an encrypted mail to the user")
print("- IP address information to check the IP is a ProtonVPN user")
print("- ProtonMail User address Internet and Dark Web traces\n\n")

# Script Information in French
print("\u001b[33m[FR] \033[1mProt1ntelligence\033[0m\u001b[33m est utile pour obtenir des infos sur:")
print("- Un compte ProtonMail et sa date de creation (la date de creation est fiable si le compte cible n'a pas fait une nouvelle cle PGP depuis la creation du compte")
print("- Cle PGP de l'utilisateur, date de creation de la cle, et telechargement pour copier et ajouter a votre KeyChain")
print("- Type de clé PGP: RSA 4096 ou ECC Curve25519")
print("- Si une adresse IP appartient a un utilisateur ProtonVPN")
print("- Les traces du mail ProtonMail de la cible (open source et sur le Dark Net)\n\n")
...
def checkprotonapistatus():
    ...
    if requestprotonmailstatus.status_code == 200:
        print(
            "\u001b[32m\033[1m\n\nGood to go! ProtonMail API is ONLINE!!!\u001b[32m \U0001F7E2 \U0001F680   |  \u001b[33m RAS! API ProtonMail est EN LIGNE!!! \033[0m \U0001F7E2 \U0001F680 \n\n")
    else:
        print(
...
\u001b[31m\U0001F575\033[1m  INTELLIGENCE COLLECTION CHOICES FOR THE PROTONMAIL TARGET: | \u001b[31mCHOIX METHODE DE COLLECTE DE RENSEIGNEMENT POUR LA CIBLE PROTONMAIL:\n

\u001b[32m\U0001F50D- \033[1mALPHA\033[0m\u001b[32m: Type alpha to check if a ProtonMail account exists  | \u001b[33m\033[1mALPHA\033[0m\u001b[33m: Tapez alpha pour verifier si un compte ProtonMail existe 

\u001b[32m\U0001F4E1- \033[1mBRAVO\033[0m\u001b[32m: Type bravo to run a search on the Proton email to check digital footprints  | \u001b[33m\033[1mBRAVO\033[0m\u001b[33m: Tapez bravo pour lancer une verification sur le mail Proton et voir l'empreinte numérique 

\u001b[32m\U0001F3F4- \033[1mCHARLIE\033[0m\u001b[32m: Type charlie to run a Dark Web search on the Proton Email  | \u001b[33m\033[1mCHARLIE\033[0m\u001b[33m: Tapez charlie pour lancer une verification Dark Net sur le mail

\u001b[32m\U0001F511- \033[1mDELTA\033[0m\u001b[32m: Type delta to get ProtonMail user PGP Key and Key creation date  | \u001b[33m\033[1mDELTA\033[0m\u001b[33m: Tapez delta pour obtenir la cle PGP de l'utilisateur et date de creation de la cle

\u001b[32m\U0001F4BB- \033[1mECHO\033[0m\u001b[32m: Type echo to verify if an IP address belongs to a ProtonVPN user | \u001b[33m\033[1mECHO\033[0m\u001b[33m: Tapez echo pour verifier si une adresse IP appartient a un utilisateur ProtonVPN\u001b[32m
"""
...
def protonmailaccountcheck():
    ...
    print(
        "\033[1m\u001b[32m\nCheck if ProtonMail account exists / \u001b[33mVerifier si compte ProtonMail existe:\033[0m\u001b[32m\n")
    while invalidEmail:

        mail = input("\033[1mEmail address + Enter : ")
...
def emailtraces():
    ...
    searchfor = input(
        """\u001b[32mEnter Target Email in quotation marks!(Example:"admin@protonmail.com")/\u001b[33m Entrez le mail de la cible dans des guillemets! (Example:"admin@protonmail.com"):\u001b[32m """)
...
def darkwebtraces():
    ...
    choice = input(
        """\033[1mView results in Browser ("B") or Terminal ("T")?/\u001b[33m Voir resultats dans Navigateur ("B") ou Terminal ("T")?\u001b[32m B/T: """)
...
def darkwebbrowser():
    ...
    """
    query = input("""\nInput Target email address or any query to search the Dark Web (example: "darkmatterproject@protonmail.com") / \u001b[33m Renseigner le mail de la cible (ou toute recherche Dark Net)\u001b[32m: """)
...
def pgpkeyinformation():
    ...
    print(
        "\033[1m\nInput the ProtonMail user email address to get the user PGP Key / \u001b[33mObtenir la cle PGP de la cible avec le mail ProtonMail\033[0m\u001b[32m\n")
    while invalidEmail:

        mail = input("\033[1mProtonMail email + Hit Enter: ")
...
def protonvpnipsearch():
    ...
    while True:
        try:
            ip = ipaddress.ip_address(input(
                '\033[1m\n\nEnter Target IP address: (Example: "185.159.157.1")  / \u001b[33mAdresse IP de la Cible: \033[0m\u001b[32m '))
            break
        except ValueError:
            continue
    ...
    if str(ip) in bodyResponse:
        print(
            "\033[1m\n\nThis IP belongs to a ProtonVPN user!!! \U0001F4A5 \U0001F4BB / \u001b[33mCette adresse IP appartient a un utilisateur ProtonVPN!!! \U0001F4A5 \U0001F4BB  \033[0m\u001b[32m ")
    else:
        print(
            "\u001b[31m\033[1m\n\nThis IP address does not belong to ProtonVPN user / Cette adresse IP n'appartient pas a un utilisateur ProtonVPN\033[0m\u001b[32m ")


def main():
    printprot1ntelligencebanner()
    choice = input(
        """\033[1m\u001b[32mType "c" to check Proton API Status / \033[1m\u001b[33mTapez "c" pour verification du statut de l'API ProtonMail: \033[0m\u001b[32m""")
    if choice == "c":
        checkprotonapistatus()
    choice = input("""\033[1mView Modules? /\u001b[33m Voir les Modules? :\u001b[32m "Y" or "N":\033[0m\u001b[32m """)
    if choice == "Y":
        printprot1ntelligenceintro()

    while True:
        choice = input(
            """\033[1mMake Choice/\u001b[33mFaire un Choix\u001b[32m:   \033[1malpha | bravo | charlie | delta | echo :\033[0m\u001b[32m """)
        if choice == "alpha":
